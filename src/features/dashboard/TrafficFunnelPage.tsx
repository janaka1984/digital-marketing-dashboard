...
export default function TrafficFunnelPage() {
  ...
  const { data: funnel, isFetching: loadingFunnel } = useGetDashboardFunnelQuery({ range });
  ...
  const ctr = totalPageviews ? ((totalClicks / totalPageviews) * 100).toFixed(2) : '0.00';
  const initRate = totalClicks ? ((totalInitiated / totalClicks) * 100).toFixed(2) : '0.00';
  const purchaseRate = totalInitiated ? ((totalPurchases / totalInitiated) * 100).toFixed(2) : '0.00';
  ...
  return (
    <Stack spacing={3}>
      <Typography variant="h4" fontWeight={700}>
        {role === 'agency' ? 'Traffic & Funnel Deep Dive (Agency)' : 'Traffic & Funnel Deep Dive'}
      </Typography>
...
function StatRow({ label, value }: StatRowProps) {
  return (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5, fontSize: 14 }}>
      <span>{label}</span>
      <span>{value}</span>
    </Box>
  );
